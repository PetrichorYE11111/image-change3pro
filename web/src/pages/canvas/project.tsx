...
import { buildNodeGenerationContext, buildNodeGenerationInputs, buildNodeResponseMessages, hydrateNodeGenerationContext, buildAssetGenerationInputs, type NodeGenerationInput } from "@/components/canvas/canvas-node-generation";
...
import { buildNodeMentionReferences, buildAssetMentionReferences, countReferencesByKind, type CanvasResourceReference } from "@/lib/canvas/canvas-resource-references";
...
function InfiniteCanvasPage() {
    ...
    const mentionReferencesByNodeId = useMemo(() => {
        ...
        nodes.forEach((node) => {
            const canvasRefs = buildNodeMentionReferences(node, nodes, connections);
            const assetRefs = buildAssetMentionReferences(assets, countReferencesByKind(canvasRefs));
            map.set(node.id, [...canvasRefs, ...assetRefs]);
        });
        return map;
    }, [connections, nodes, assets]);
    ...
    const maskEditImageNode = useCallback(
        async (node: CanvasNodeData, payload: CanvasImageMaskEditPayload) => {
            ...
            const extraRefs = payload.extraReferences ?? [];
            // 提示词说明：额外参考图用编号引用，OpenAI 走 mask 时也能靠文字理解
            const refText = extraRefs.length ? `可参考随附的 ${extraRefs.length} 张参考图（图片1..${extraRefs.length}）的内容或风格。` : "";
            const prompt = t("canvas.projectPage.maskPrompt", { prompt: `${refText}${userPrompt}` });
            const childId = nanoid();
            const source = { id: node.id, name: `${node.title || node.id}.png`, type: node.metadata.mimeType || "image/png", dataUrl: node.metadata.content, storageKey: node.metadata.storageKey };
            // references[0] 必须是待编辑原图；额外参考图追加其后，供 Gemini 合成与多图参考使用
            const editReferences = [source, ...extraRefs];
            ...
            try {
                const image = await requestEdit(generationConfig, prompt, editReferences, { id: `${node.id}-mask`, name: "mask.png", type: "image/png", dataUrl: payload.maskDataUrl }, { signal: controller.signal }).then((items) => items[0]);
    ...
    const handleGenerateNode = useCallback(
        async (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string) => {
            ...
            const generationContext = await hydrateNodeGenerationContext(
                buildNodeGenerationContext(nodeId, nodesRef.current, connectionsRef.current, editingTextNode ? t("canvas.projectPage.editTextPrompt", { source: sourceTextContent, prompt }) : prompt, buildAssetGenerationInputs(assetsRef.current)),
            );
            ...
            try {
                if (mode === "image") {
                    ...
                    // 直接编辑已有图片节点时，输出跟随原图比例，避免 auto 被 Gemini 默认成方图
                    const editSize = sourceReference.length ? resolveEditSize(sourceNode) : undefined;
                    // 把 editSize 写进元数据，重试时也能保持原图比例
                    const generationMetadata = buildImageGenerationMetadata(generationType, editSize ? { ...generationConfig, size: editSize } : generationConfig, count, referenceImages);
                        ...
                        imageIds.map(async (imageId) => {
                            try {
                                const image = referenceImages.length
                                    ? await requestEdit({ ...generationConfig, count: "1", ...(editSize ? { size: editSize } : {}) }, effectivePrompt, referenceImages, undefined, { signal: controller.signal }).then((items) => items[0])
    ...
    const handleRetryNode = useCallback(
        async (node: CanvasNodeData, imageId?: string) => {
            ...
            const context = hasSavedImageMetadata ? null : await hydrateNodeGenerationContext(buildNodeGenerationContext(sourceNode.id, nodesRef.current, connectionsRef.current, sourceNode.metadata?.prompt || node.metadata?.prompt || "", buildAssetGenerationInputs(assetsRef.current)));
